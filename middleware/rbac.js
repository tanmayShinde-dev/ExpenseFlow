...
function checkPermission(requiredPermission, options = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      // Hierarchical Permission Check (#629)
      ...
      for (const permission of permissions) {
        if (await workspaceService.checkHierarchicalPermission(userId, workspaceId, permission)) {
          hasPermission = true;
          break;
        }
      }

      // Fallback for self-action if hierarchy fails
      if (!hasPermission && allowSelf) {
      ...
      if (!hasPermission) {
        return res.status(403).json({
          error: 'You do not have permission to perform this action (check parent entities if applicable)',
          code: 'PERMISSION_DENIED',
          required: permissions,
          userRole: workspace.getUserRole(userId) || 'none'
        });
      }
...
function workspaceAccess(options = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      // Check if user has access (Hierarchical #629)
      const isOwner = workspace.owner._id.toString() === userId.toString();
      const hasAccess = isOwner || await workspaceService.checkHierarchicalPermission(userId, workspaceId, 'workspace:view');

      if (!hasAccess) {
        return res.status(403).json({
          error: 'You do not have access to this workspace or its parents',
          code: 'ACCESS_DENIED'
        });
      }

      req.workspace = workspace;
      req.userRole = workspace.getUserRole(userId) || 'collaborator'; // Fallback for hierarchical access
      req.isOwner = isOwner;
